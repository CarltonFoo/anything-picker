...
const jobs = clinicSubset.map((id, i) => queue.push(fetchClinicInfo, id, clinicLocation.length - i))

Promise.all(jobs).then(() => {
  if (debug.length > 0) console.log('Unsuccessfull:', debug)
})

const clinicName = clinicLocation.forEach(i => {
  return clinicLocation[i]
})
...
function fetchClinicInfo (id, k) {
  ...
  return axios({
    method: 'get',
    url: 'https://www.singhealth.com.sg/PatientCare/GP/Pages/' + clinicName + '.aspx',
    responseType: 'text'
  }).then(res => cheerio.load(res.data))
    .then(parseClinicInfo)
    .then(result => {
      fs.writeFile(`data/singhealthraw/${clinicName}.json`, JSON.stringify(result, null, '\t'))
    })
    .catch(err => {
      debug.push(id)
